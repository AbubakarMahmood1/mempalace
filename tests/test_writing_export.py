...
def test_writing_search_modes_preserve_room_priority(mode, expected_rooms, monkeypatch):
    ...
    monkeypatch.setattr("mempalace.writing_export.search_memories", fake_search_memories)
...
def test_writing_sync_cli_respects_sync_policy(monkeypatch, capsys):
    ...
    import mempalace.writing_export as writing_export
    ...
    try:
        ...
        monkeypatch.setattr(writing_export, "get_writing_sidecar_status", lambda **kwargs: clean_status)
        monkeypatch.setattr(
            writing_export,
            "export_writing_corpus",
        ...
        monkeypatch.setattr(
            sys,
            "argv",
            ["mempalace", "writing-sync", str(tmp_path), "--project", "Witcher-DC"],
        )
        cli.main()
        ...
        monkeypatch.setattr(writing_export, "get_writing_sidecar_status", lambda **kwargs: stale_status)
        monkeypatch.setattr(
            sys,
            "argv",
            [
...
def test_writing_search_cli_rebuilds_if_needed(monkeypatch, capsys):
    ...
    try:
        ...
        monkeypatch.setattr(writing_export, "get_writing_sidecar_status", fake_status)
        monkeypatch.setattr(writing_export, "export_writing_corpus", fake_export)
        monkeypatch.setattr(
            writing_export,
            "search_writing_sidecar",
            lambda **kwargs: {
